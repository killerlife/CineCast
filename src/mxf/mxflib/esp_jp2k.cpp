 ...
 *
 *	\version $Id: esp_jp2k.cpp,v 1.1.1.1 2007/04/03 02:08:42 baiqian Exp $
 *
 */
/*
...
namespace
{
	...
	const char* pJpiFooterEndFlag="Ta_Jpc Index IsEnd!";
	const char* pJpiFooterBegFlag="Ta_Jpc Index Start!";
	const int  JPIFooterEndFlagLen=strlen(pJpiFooterEndFlag)+1;
	const int  JPIFooterBegFlagLen=strlen(pJpiFooterBegFlag)+1;
...
EssenceStreamDescriptorList mxflib::JP2K_EssenceSubParser::IdentifyEssence(FileHandle InFile)
{
	...
	if( memcmp(Buffer, JP2_Signature, 12) != 0 )
	{
		...
		//baiqian : is jpi file?, jpeg2000 codestream file with a indextable desinged by WangDongLiang of TaMedia
		ParseJpiFooter(InFile);
		return Ret;
	}
...
DataChunkPtr mxflib::JP2K_EssenceSubParser::Read(FileHandle InFile, UInt32 Stream, UInt64 Count /*=1*/)
{
	...
	//baiqian add: temp check code;
	const UInt8 J2C_Signature[] = { 0xff, 0x4f, 0xff, 0x51 };
	ASSERT( memcmp(Ret->Data, J2C_Signature, 4) == 0);
...
MDObjectPtr mxflib::JP2K_EssenceSubParser::BuildDescriptorFromCodeStream(FileHandle InFile, Position Offset /*=0*/)
{
    ...
    //TODO: baiqian add ,need set PictureEssenceCoding_UL,reference s422m-5.5 and MXF JPEG2000 Application for D-Cinema 6.2.2
	UInt8 J2k_2k_Coding[16] = {0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x07,0x04,0x01,0x02,0x02,0x03,0x01,0x01,0x03};
    UInt8 J2k_4k_Coding[16] = {0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x07,0x04,0x01,0x02,0x02,0x03,0x01,0x01,0x04};
	//if is 2k
	if(Width >2048)
	{
		DataChunk J2k_4k_Coding_UL;
		J2k_4k_Coding_UL.Set(16, J2k_4k_Coding);
		Ret->SetValue(PictureEssenceCoding_UL,J2k_4k_Coding_UL);
	}
	else
	{
		DataChunk J2k_2k_Coding_UL;
		J2k_2k_Coding_UL.Set(16, J2k_2k_Coding);
		Ret->SetValue(PictureEssenceCoding_UL,J2k_2k_Coding_UL);
	}
...
bool mxflib::JP2K_EssenceSubParser::SetOriginAndDuration(FileHandle InFile,Position EditUnit,Position Duration)
{
	ASSERT(PictureNumber ==0);
	if(EditUnit>0)
	{
		if(CurrentPos == 0) CurrentPos = DataStart;
		Position picNumber=0;
		Length Ret=0;
		if( !m_isJpi)
			Ret=CountPos(InFile, CurrentPos, EditUnit, picNumber);
		else
            Ret=CountJpiPos(InFile, 0, EditUnit, picNumber);
		ASSERT(picNumber == EditUnit);
		DataStart+=Ret;
		m_startUnit=EditUnit;
	}
	if(Duration>0)
	{
		//TODO: check Duration if valid?
		m_Duration=Duration;
	}
	return true;
}
...
{ 
    ...
    if( !m_isJpi)
		Ret=CountPos(InFile,CurrentPos,Count,picNumber);
	else
		Ret=CountJpiPos(InFile,m_startUnit+PictureNumber,Count,picNumber);
	if( Manager )
	{
		for(Position i=0;i<picNumber;++i)
		{
			// Offer this index table data to the index manager
			Manager->OfferEditUnit(ManagedStreamID, PictureNumber, 0, 0x80);
			Manager->OfferTemporalOffset(PictureNumber, 0);
			++PictureNumber;
		}
	}
	else 
...
Length mxflib::JP2K_EssenceSubParser::CountJpiPos(FileHandle InFile,Position Pos,Length Count,Position& picNumber)
{
	Length Ret = 0;
	picNumber=0;
	if(m_isJpi && Pos < m_jpiIndexInfo.editUnitNum)
	{ 
		UInt64 filePos=m_jpiIndexInfo.fileLen -m_jpiIndexInfo.indexLen+JPIFooterEndFlagLen;
		filePos += ( sizeof(Int64)+sizeof(Int32) ) * Pos;
		FileSeek(InFile, filePos);
		UInt64 unitPos=0;
		UInt32  unitLen=0;
		while(Count >0 )
		{
            unitPos=ReadU64(InFile);
            unitLen=ReadU32(InFile);
			Ret+=unitLen;
			++picNumber;
			if( (Pos+picNumber) >= m_jpiIndexInfo.editUnitNum)
				break;
			//else break;
			--Count;
		}
...
Length mxflib::JP2K_EssenceSubParser::CountPos(FileHandle InFile,Position Pos,Length Count,Position& picNumber)
{
	Length Ret = 0;
	...
	//		}
		...
		for(;;)
		{
			...
			if(MarkerSegments[Marker])
			{
				...
				// Read the length
				UInt32 SegmentLength = GetU16(p);
				ASSERT(SegmentLength > 2);
				//printf("  Length = 0x%04x\n", SegmentLength);
            
				//baiqian add: for SOT ,there are data fellowed;
				//so need count length of tile part;
				if(Marker == 0x90 )
				{
					SegmentLength=GetU32(p+4)-2;
				}
...
bool mxflib::JP2K_EssenceSubParser::ParseJpiFooter(FileHandle InFile)
{

	
/*
		??????????	????????????ID			????????????				??????????	??????		????????????		????????????ID
		0xff4f??	pJpiFooterBegFlag	   (pos,len)(pos,len)..		 File_Length(64bit)	Frame_Total_Num(32bit)	Ender_Length(32bit)	pJpiFooterEndFlag
*/
	...
	FileSeekEnd(InFile);
	UInt64 EndPos,Pos;
	EndPos =Pos = FileTell(InFile);
	FileSeek(InFile, Pos-JPIFooterEndFlagLen);
	Pos = FileTell(InFile);
	BufferBytes = (int)FileRead(InFile, Buffer, JPIFooterEndFlagLen);
	if(memcmp(Buffer,pJpiFooterEndFlag,JPIFooterEndFlagLen) ==0 )
	{
		int size=sizeof(JPIIndex);
		FileSeek(InFile, Pos-size);
		m_jpiIndexInfo.fileLen=ReadU64(InFile);
		m_jpiIndexInfo.editUnitNum=ReadU32(InFile);
		m_jpiIndexInfo.indexLen=ReadU32(InFile);

		//FileRead(InFile,(unsigned char*)&m_jpiIndexInfo,sizeof(JPIIndex));
		//seek end
		if(m_jpiIndexInfo.fileLen != EndPos)
			return false;
		FileSeek(InFile, EndPos-m_jpiIndexInfo.indexLen);
        BufferBytes = (int)FileRead(InFile, Buffer, JPIFooterBegFlagLen);
		if(memcmp(Buffer,pJpiFooterBegFlag,JPIFooterBegFlagLen) !=0 )
			return false;
		if(m_jpiIndexInfo.indexLen !=(m_jpiIndexInfo.editUnitNum* (sizeof(UInt64)+sizeof(UInt32))+JPIFooterBegFlagLen+JPIFooterEndFlagLen+sizeof(JPIIndex) ) )
			return false;
...
UInt64 mxflib::JP2K_EssenceSubParser::GetDuration()
{
	if(m_Duration)
		return m_Duration;
	if(m_isJpi)
		return m_jpiIndexInfo.editUnitNum;
	return 0;
}
