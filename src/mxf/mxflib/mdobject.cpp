 ...
 *			the XML dictionary.
 *
 *	\version $Id: mdobject.cpp,v 1.2 2007/09/20 05:47:13 baiqian Exp $
 *
 */
/*
...
bool MDObject::IsA(const UL &BaseType)
{
	MDOTypePtr TestType = Type;
	//baiqian modify, :
	//std::string baseName=BaseType.GetString();
	while(TestType)
	{
	//	std::string name=TestType->FullName();
		const ULPtr& ul=TestType->GetTypeUL();
		if(!ul)	return false;
		if(*ul == BaseType)
		{
			return true;
