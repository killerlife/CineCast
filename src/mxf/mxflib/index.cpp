 ...
 *
 *	\version $Id: index.cpp,v 1.3 2007/09/21 03:45:43 baiqian Exp $
 *
 */
/*
...
IndexPosPtr IndexTable::LookupFromDataChunk(bool& find , DataChunkPtr indexTableChunk, Position EditUnit, int SubItem /* =0 */, bool Reorder /* =true */)
{
	find = false;
	...
	char key[16];
	memcpy(key, indexTableChunk->Data, 16);
	UInt64 offset = 0;
	UInt8 *p = 0;
	Length Bytes = 0;
	Int64 StartPosition, EditUnitDur;
	
	do
	{
		p = indexTableChunk->Data + offset + 16;
		Bytes = indexTableChunk->Size -offset -16;
	
		UInt8 *Prev_p = p;
		int ItemLength = ReadBER(&p, (int)Bytes);
		Bytes -= (p - Prev_p);
		//error("LookupFromDataChunk: ItemLength =%ld,%d, total=%ld\n",ItemLength,(p - Prev_p), indexTableChunk->Size);
		if(memcmp(key, indexTableChunk->Data + offset, 16) == 0)
		{
	
			EditUnitByteCount = GetU32(p + (4+16) + (4+8)*3 +4);
			StartPosition = GetU64(p + (4+16) + (4+8) +4 );//Segment->GetInt64(IndexStartPosition_UL);
			EditUnitDur = GetU64(p + (4+16) + (4+8)*2 + 4 );//Segment->GetInt64(IndexDuration_UL);
			//error("LookupFromDataChunk: seek to =%ld\n", (UInt32)EditUnit);
			//error("LookupFromDataChunk: EditUnitByteCount =%ld\n",(UInt32)EditUnitByteCount);
			//error("LookupFromDataChunk: StartPosition =%ld\n",(UInt32)StartPosition);
			//error("LookupFromDataChunk: EditUnitDur =%ld\n",(UInt32)EditUnitDur);
			if(EditUnitByteCount)
				break;
                   if(EditUnitDur == 0)
				return Ret;
			if(EditUnit >= StartPosition   
			   && EditUnit < (StartPosition + EditUnitDur))
			break;

		}
		else
		{
			//error("LookupFromDataChunk: not indextable,ItemLength=%ld\n",ItemLength);
			return Ret;
		}
		offset += 20 + ItemLength;
	
	}while(1);
	//baiiqan: now, just do it for one essence.
	if(SubItem > 0)
		return Ret;
	// Deal with CBR first
	if(EditUnitByteCount)
	{
		...
		else
		{
			...
			else
			{
				...
				//baiqian: now, don't support this:
				ASSERT(0);
				/*
	...
	/*if(EditUnitDur == 0
	   || StartPosition >  EditUnit
		|| (StartPosition + EditUnitDur -1) < EditUnit)
	return Ret;
	*/
	NSL = GetU8(p + (4+16) + (4+8)*3 + (4+4)*3 +4);//Segment->GetUInt(SliceCount_UL);
       p = p + (4+16) + (4+8)*3 + (4+4)*3 + (4+1);
	NPE = 0;
	if(p[0] == 0x3F && p[1] == 0x0E)
	{
		NPE = GetU8(p+4);//Segment->GetUInt(PosTableCount_UL);
		p += 4 + 1;
	}
	IndexEntrySize = (11 + 4*NSL + 8*NPE);
	UInt8 * pDeltaArrayPtr = p;
	int Delta_PosTableIndex = 0;
	UInt32 Delta_Slice =0 , Delta_ElementDelta = 0;
	if(pDeltaArrayPtr[0] == 0x3F && pDeltaArrayPtr[1] == 0x09)
	{
		p += 2;
       UInt16 DeltaArraySize = GetU16(p);
		p += 2;
		//UInt32 Delta_NDE = GetU32(p);
		GetU32(p);
		p += 4;
		UInt32 Delta_Size = GetU32(p);
		p += 4;
		BaseDeltaCount = (DeltaArraySize - 4*2)/Delta_Size;
		
		Delta_PosTableIndex = GetI8(p);
		p += 1;
		Delta_Slice = GetU8(p);
		p += 1;
		Delta_ElementDelta = GetU32(p);
		p += 4;
		
	}
	else
		BaseDeltaCount = 0;
	Int8 TemporalOffset = 0;
	UInt8 * pIndexArrayPtr = p;
	
	if(pIndexArrayPtr[0] == 0x3F && pIndexArrayPtr[1] == 0x0A)
	{
	
		p += 2;
       	UInt16 IndexArraySize = GetU16(p);
		p += 2;
		//UInt32 Index_NDE = GetU32(p);
		GetU32(p);
		p += 4;
		UInt32 Index_Size = GetU32(p);
		p += 4;
		UInt32 IndexCount = (IndexArraySize - 4*2)/Index_Size;
		
		UInt8 * pIndex = p + (EditUnit - StartPosition)*Index_Size;
		TemporalOffset = GetI8(pIndex);
		Ret->KeyFrameOffset = GetI8(pIndex + 1);
		Ret->Flags = GetU8(pIndex + 2);
		Ret->Location = GetU64(pIndex + 3);
		if(Ret->KeyFrameOffset > EditUnit - StartPosition )
		{
			// Key Frame is in a different Index Table Segment
			printf("!!!!!!!!!!KeyFrameOffset=%d!\n",Ret->KeyFrameOffset);
			//FIXME: for V15, should get next Key frame, and Ret->KeyFrameOffset shall be (-1)* offset
			//but, we just have one Index Table segment as spec!.
			Ret->KeyLocation = ~0;
		}
		else if(Ret->KeyFrameOffset > 0)
		{
	
			pIndex = p + (EditUnit - StartPosition - Ret->KeyFrameOffset)*Index_Size;
			Ret->KeyLocation = GetU64(pIndex + 3);
		}
		else 
			Ret->KeyLocation = Ret->Location;
		//printf("TemporalOffset=%d!\n",TemporalOffset);
		//error("LookupFromDataChunk: Ret->KeyFrameOffset=%ld, Ret->Location =%lld,Ret->KeyLocation =%lld\n",Ret->KeyFrameOffset, Ret->Location,Ret->KeyLocation);
		
	}
	else
		return Ret;
	// Apply temporal re-ordering if we should, but only if we have details of the exact sub-item
	if(Reorder && (TemporalOffset != 0) && (SubItem < BaseDeltaCount) && (Delta_PosTableIndex < 0))
	{
	//	printf("reorder, TemporalOffset=%d!\n",TemporalOffset);
		return LookupFromDataChunk(find, indexTableChunk, EditUnit + TemporalOffset, SubItem, false);
	}
...
/*
IndexPosPtr IndexTable::LookupFile(bool& find , MDObjectPtr Segment, Position EditUnit, int SubItem, bool Reorder )
{
	...
	Int64 EditUnitDur = Segment->GetInt64(IndexDuration_UL);
//	error("Lookupfile: EditUnitByteCount =%ld\n",EditUnitByteCount);
//	error("Lookupfile: StartPosition =%ld\n",StartPosition);
//	error("Lookupfile: EditUnitDur =%ld\n",EditUnitDur);
	//baiiqan: now, just do it for one essence.
	if(SubItem > 0)
		return Ret;
	// Deal with CBR first
	...
	IndexEntrySize = (11 + 4*NSL + 8*NPE);
	//error("IndexEntrySize =%ld, NSL=%ld, NPE=%ld \n", IndexEntrySize, NSL , NPE);
	
	MDObjectPtr DeltaArrayPtr = Segment[DeltaEntryArray_UL];
	int Delta_PosTableIndex = 0;
	UInt32 Delta_Slice =0 , Delta_ElementDelta = 0;
	...
	{
		BaseDeltaCount = DeltaArrayPtr->size() / 3;		// << There are 3 items in each DeltaCount entry
		MDObjectULList::iterator it = DeltaArrayPtr->begin();
		while(it != DeltaArrayPtr->end())
		{
				Delta_PosTableIndex = (*it).second->GetInt();
				++it;
				Delta_Slice = (*it).second->GetUInt();
				++it;
				Delta_ElementDelta = (*it).second->GetUInt();
				it++;
				break;
		}
	}
	//printf("BaseDeltaCount = %ld, Delta_PosTableIndex= %d, DeltaArrayPtr->size() = %ld\n" , BaseDeltaCount, Delta_PosTableIndex, DeltaArrayPtr->size());

	MDObjectPtr IndexArrayPtr = Segment[IndexEntryArray_UL];
	//printf("IndexArrayPtr->Size() = %ld, %ld\n" , IndexArrayPtr->size(), IndexArrayPtr->size()/2016);
	
	Int8 TemporalOffset = 0;
	MDObjectULList::iterator it = IndexArrayPtr->begin();
	Int64 num = 0;
	while(it != IndexArrayPtr->end())
	{
		if(num == (EditUnit - StartPosition)*6)
		{
			   //printf("num=%ld\n", num);
				//printf("offset = %d   ", (*it).second->GetInt());
				TemporalOffset = (Int8)(*it).second->GetInt();
				++it;
				Ret->KeyFrameOffset = (*it).second->GetInt();
				//printf("key frmame offset = %d   ", (*it).second->GetInt());
				++it;
				Ret->Flags = (*it).second->GetInt();
				//printf("flag = %d   ", (*it).second->GetInt());
				++it;
				Ret->Location = (*it).second->GetUInt64();
				//printf("stream offset = %ld ", (*it).second->GetUInt64());
				++it;
				//printf("slice offset = %ld \n", (*it).second->GetUInt());
				++it;
				//printf("post offset = %ld \n", (*it).second->GetInt());
				++it;
				if(Ret->KeyFrameOffset > EditUnit - StartPosition )
				{
					// Key Frame is in a different Index Table Segment
				...
				{
					for(int i = 0; i < Ret->KeyFrameOffset * 6 ;++i)
						--it;
					++it;
					++it;
					++it;
					Ret->KeyLocation = (*it).second->GetUInt64();
				}
				else 
					Ret->KeyLocation = Ret->Location;
				break;
	...
	{
		//printf("reorder!\n");
		return LookupFile(find, Segment, EditUnit + TemporalOffset, SubItem, false);
	}
...
//	warning("Index table reading not complete!\n");
	...
	else //baiqian add: check index sid and body sid
	{
		UInt32 indexSID = Segment->GetUInt(IndexSID_UL);
		UInt32 bodySID = Segment->GetUInt(BodySID_UL);
		if(indexSID != IndexSID || bodySID!=BodySID )
		{
			error("error indexSID or bodySID When add Index Segment in %s at %s\n", Segment->FullName().c_str(), Segment->GetSourceLocation().c_str());
			return Ret;
		}
	}
	...
	else // VBR
	{
		...
		IndexEntrySize = (11 + 4*NSL + 8*NPE);
		error("look up: IndexEntrySize =%ld, NSL=%ld, NPE=%ld \n", IndexEntrySize, NSL , NPE);
