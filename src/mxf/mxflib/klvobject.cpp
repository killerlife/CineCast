 ...
 *
 *	\version $Id: klvobject.cpp,v 1.2 2007/09/20 05:47:13 baiqian Exp $
 *
 */
/*
...
//baiqian add: for read klv,not v
Length KLVObject::ReadDataEx(Position Offset , Length Size) 
{
	// Delagate to ReadHandler if defined
	if(ReadHandler) return ReadHandler->ReadData(Data, this, Offset, Size);
	if(Source.Offset < 0)
	{
		error("Call to KLVObject::Base_ReadDataFrom() with no read handler defined and DataBase undefined\n");
		return 0;
	...
	// Initially plan to read all the bytes available
	Length BytesToRead = Source.OuterLength + Source.KLSize - Offset;
