...
MyTestReaderCase::MyTestReaderCase()
{
   ...
   m_pIniFile =createMyini();
    ...
    m_pIniFile->load("testDcpDump.txt") ;
   ...
   readIni(*m_pIniFile,indexName.c_str(), "StartTime", m_StartTime);
   readIni(*m_pIniFile,indexName.c_str(), "EndTime", m_EndTime);
	if( m_EndTime<m_StartTime){
		m_EndTime=m_StartTime;
	}
	m_EndTime = m_EndTime * 24 ;
	m_StartTime = m_StartTime * 24 ;
   string keystr;
...
void MyTestReaderCase::testOpen()
{
	...
	if(m_pReader->open(m_compositionFile.c_str(),m_packingFile.c_str()))
		m_pReader->close();
}
void MyTestReaderCase::testOpenFail()
{

	if( !m_pReader->open(m_compositionFile.c_str(),"nothisfile.txt") )
		cout << m_pReader->getLastError() <<endl;
	if( !m_pReader->open("nothisfile.txt",m_packingFile.c_str()) )
		cout << m_pReader->getLastError() <<endl;
...
void MyTestReaderCase::testDumpPicture()
{
	...
	//return;
	if( !m_pReader->open(m_compositionFile.c_str(),m_packingFile.c_str()) ){
		cout<<"Open Source File ["<<m_compositionFile<<"] or ["<<m_packingFile<<"]Error"<<endl;	
		return;
	}
	mxflib::UInt64 dur=m_pReader->duration();
	int num=0;
...
cout<<"Start: "<<m_StartTime<<endl;	
cout<<"End: "<<m_EndTime<<endl;	
	
num = m_StartTime ;
	m_pReader->seek(num);
	for(;num< (int)dur && num<m_EndTime;++num)
	//for(;num< (int)dur;++num)
	{
		FrameData vData= m_pReader->readVideoFrame();
		if(num<m_StartTime)continue;	
		if(vData.empty()){
			continue;
		}
		char savefile[200];
...
void MyTestReaderCase::testDumpAudio()
{
	...
	{
		cout<<"Create WaveFile ["<<savefile<<"] Error !"<<endl;
		return;	
	}	
...
void usage(void)
{
	cout<<"Usage: testDcpDump [options]"<<endl;
	cout<<"-DA : DumpAudio"<<endl;
	cout<<"-DP : DumpPicture"<<endl;
	cout<<"-DPF : DumpPictureFile"<<endl;
}
...
int main(int argc, char *argv[])
{
	...
	MyTestReaderCase* mydo = NULL ;
	
	if( argc == 1 ){
		usage();
	}

	while((myarg = getopt(argc,argv,"D:") ) != -1 ){
		switch(myarg){
			case 'D':
					if(mydo==NULL){
						mydo = new MyTestReaderCase();
					}

					if(strcmp(optarg,"A")==0){
							mydo->testDumpAudio();
					}else if(strcmp(optarg,"P")==0){
							mydo->testDumpPicture();
					}else if(strcmp(optarg,"PF")==0){
							mydo->testDumpPictureFile();
					}
					break;
			default:
					usage();
