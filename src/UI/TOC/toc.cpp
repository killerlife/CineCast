...
{         
	...
	server_login = new CTcpServer(this,8000);     //登陆处理
...
// 	connect(server_login,SIGNAL(signal_clientChange()),this,SLOT(updateClientList()));
	...
	//new
	server_main = new CTcpServer(this,10099);   //主服务
...
// 	connect(server_main,SIGNAL(signal_clientChange()),this,SLOT(updateClientList()));
	...
	//关联信号直接操作socket线程，CTcpServer作为中转
...
void TOC::slot_CMD_SocketToUI_process(int SocketID,int cmdtype,int val)   //socket发过来的命令处理
{
	switch(cmdtype)
	{
...
void TOC::slot_CMD_TcpServertoUI_process(int SocketID,int cmdtype)       //服务器发给UI
{
	switch(cmdtype)
	{
...
void ContentItem::MakeItem()
{
	...
	pData[1]=AuthenRequest.SoftVersion;  //保证是字符串
	pData[2]=AuthenRequest.Time_Login;   //保证是字符串
	...
	switch(flag)
	{
	case 0: setText(8,"IDLE");    break;  //空虚
	case 1: setText(8,"Waiting"); break;  //正在升级
	case 2: setText(8,"OK");    break;  //升级完成
	default:	break;
	}
}
...
void TOC::on_pushButtonMD5_clicked()
{
	...
	if(item == NULL)
		return;
	int SockID=item->tcpClientSocket->socketID;   //不能调用函数，调用变量
