...
static uint32 crc_tab32[256];//CRC查询表

static void init_crc32_tab(void);//生成CRC查询表
//uint32 Get_CRC32_fromBuffer(uint32 crcinit, uint8 * bs, uint32 bssize);//获得CRC
//uint32 GetCRC32_fromFile(FILE *fd);//获得文件CRC
...
/*计算buffer的crc校验码*/
/*
uint32 Get_CRC32_fromBuffer(uint32 crcinit, uint8 * bs, uint32 bssize)
{
    uint32 crc = crcinit^0xffffffff;
...
uint32 Get_CRC32_fromBuffer(uint32 crcinit, uint8 *pData,
				  int32 nLength)
{
...
//计算大文件的CRC校验码
static uint32 GetCRC32_fromFile(FILE *fd)
{
    const uint32 size = 16 * 1024;
    uint8 crcbuf[16*1024];
    uint32 rdlen;
    uint32 crc = 0;//CRC初始值为0

    while((rdlen = fread(crcbuf, sizeof(uint8), size, fd)) > 0)
    crc = Get_CRC32_fromBuffer(crc, crcbuf, rdlen);
...
*/
    ...
    printf("buf:%lld\n",Get_CRC32_fromBuffer(0,(unsigned char*)str,strlen(str)));

    //%X %x  分别用大小写表示十六进制
    printf("buf:%X\n",Get_CRC32_fromBuffer(0,(unsigned char*)str,strlen(str)));
    FILE *fd= fopen("mytest.txt","r");   // 打开文件

    unsigned int value=0;
    value= GetCRC32_fromFile(fd);
    printf("File:CRC: %X\n",value);
