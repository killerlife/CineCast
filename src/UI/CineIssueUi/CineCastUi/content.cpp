...
Content::Content(QTcpSocket* pSocket, QWidget *parent)
	: QWidget(parent), pSocket(NULL), p(parent)
{
	ui.setupUi(this);
	Init();
...
void Content::Init()
{
	ui.pushButton_ExporttoUSB->setStyleSheet("QPushButton{font-size: 14px; font-family:'Book Antiqua';}");
	ui.pushButton_DeleteDir_HDD->setStyleSheet("QPushButton{font-size: 14px; font-family:'Book Antiqua';}");
	ui.pushButton_Refush_Disk->setStyleSheet("QPushButton{font-size: 14px; font-family:'Book Antiqua';}");
	ui.pushButton_USB_UnMount->setStyleSheet("QPushButton{font-size: 14px; font-family:'Book Antiqua';}");
	ui.pushButton_DeleteDir_USB->setStyleSheet("QPushButton{font-size: 14px; font-family:'Book Antiqua';}");
	ui.treeWidget_HDD_info->setStyleSheet("QTreeWidget{font-size: 14px; font-family:'Book Antiqua';}");
	ui.treeWidget_USB_info->setStyleSheet("QTreeWidget{font-size: 14px; font-family:'Book Antiqua';}");
	ui.progressBar_CopyDir->setStyleSheet("QProgressBar{font-size: 14px; font-family:'Book Antiqua';}");
...
// 	ui.treeWidget_HDD_info->setColumnWidth(0, 45);
	ui.treeWidget_HDD_info->setColumnWidth(0, 40);
	ui.treeWidget_HDD_info->setColumnWidth(1, 310);
	ui.treeWidget_HDD_info->setColumnWidth(2, 75);
	ui.treeWidget_HDD_info->setColumnWidth(3, 65);
	ui.treeWidget_HDD_info->setColumnWidth(4, 95);
	ui.treeWidget_HDD_info->setColumnWidth(5, 95);

	ui.treeWidget_USB_info->setColumnWidth(0, 500);
#endif

	ui.groupBox->setStyleSheet("QGroupBox{font-size: 14px; font-family:'Book Antiqua';}");
	ui.label->setStyleSheet("QLabel{font-size: 14px; font-family:'Book Antiqua';}");
	...
	ui.groupBox_2->setStyleSheet("QGroupBox{font-size: 14px; font-family:'Book Antiqua';}");
	ui.label_7->setStyleSheet("QLabel{font-size: 14px; font-family:'Book Antiqua';}");
	...
	ui.label_10->setStyleSheet("QLabel{font-size: 14px; font-family:'Book Antiqua';}");
	ui.label_11->setStyleSheet("QLabel{font-size: 14px; font-family:'Book Antiqua';}");
	ui.label_12->setStyleSheet("QLabel{font-size: 14px; font-family:'Book Antiqua';}");
	ui.label_Progress->setVisible(false);
	ui.label_13->setStyleSheet("QLabel{font-size: 18px; font-family:'Book Antiqua';}");
	...
	ui.treeWidget_USB_info->setVisible(false);
	ui.progressBar_CopyDir->setVisible(false);
	ui.pushButton_ExporttoUSB->setVisible(false);
	ui.pushButton_Refush_Disk->setVisible(false);
	ui.pushButton_DeleteDir_USB->setVisible(false);
	ui.pushButton_USB_UnMount->setVisible(false);
#endif
}
...
void Content::timerEvent(QTimerEvent * te)
{
	...
	if(!bFormat)
	{
		ui.pushButton_DeleteDir_HDD->setEnabled(true);
		ui.label_13->setText("");
	}
	else
	{
		ui.pushButton_DeleteDir_HDD->setEnabled(false);
		ui.label_13->setText(tr("Can not delete content while formating."));
	}
}
...
void Content::LoadContent_HDD()
{
	...
	int i;
	int count = ui.treeWidget_HDD_info->topLevelItemCount();
	...
	if (pKL->m_pkgHead == 0x7585 && pKL->m_keyID == M_GET_HDD_INFO)
	{
		memcpy(&di_hdd, buf + sizeof(KL), sizeof(di_hdd));
		{
			...
			ui.label_HDD->setEnabled(true);

		QString s = QString::number(di_hdd.nTotal/1024/1024);
		...
		s = QString::number(di_hdd.nAvali/1024/1024);
		n = s.size();
		for(int i = 1; i <= (n-1)/3; i++)
		{
			s.insert(n - i * 3, ",");
		...
		s = QString::number((di_hdd.nTotal - di_hdd.nAvali)/1024/1024);
		n = s.size();
		for(int i = 1; i <= (n-1)/3; i++)
		{
			s.insert(n - i * 3, ",");
...
//         item_HDD[i]= new ContentItem;
		...
		while(len > 0)
		{
			...
			ContentItem *item_HDD;

			switch(sl->m_sID)
			{
			case CONTENT_ID:
				item_HDD = new ContentItem;
				item_HDD->pData[C_ID] = tmp;
				break;
			case CONTENT_NAME:
				item_HDD->pData[C_NAME] = tmp;
				break;
			case CONTENT_PROGRESS:
				item_HDD->pData[C_PROGRESS] = tmp + "%";
				break;
			case CONTENT_STATUS:
				item_HDD->pData[C_STATUS] = tmp;
				break;
			case CONTENT_FORMAT:
				item_HDD->pData[C_FORMAT] = tmp;
				break;
			case CONTENT_FILMLENGTH:
				item_HDD->pData[C_FILM_LENGTH] = tmp;
				break;
			case CONTENT_PROGRAMLENGTH:
				item_HDD->pData[C_PROGRAM_LENGTH] = tmp;
				break;
			case CONTENT_STEREOSCOPIC:
				item_HDD->pData[C_STEREO] = tmp;
				break;
			case CONTENT_ISSUER:
				item_HDD->pData[C_ISSUER] = tmp;
				break;
			case CONTENT_ISSUEDATE:
				item_HDD->pData[C_ISSUEDATE] = tmp;
				break;
			case CONTENT_TIMERANGE:
				item_HDD->pData[C_TIMERANGE] = tmp;
				break;
			case CONTENT_RECVSEGMENT:
				item_HDD->pData[C_RECVSEGMENT] = tmp;
				break;
			case CONTENT_TOTALSEGMENT:
				item_HDD->pData[C_TOTAL_SEGMENT] = tmp;
				break;
			case CONTENT_RECV_DATETIME:
				item_HDD->pData[C_RECV_DATETIME] = tmp;
				break;
			case CONTENT_LOCATE:
				...
				//ݵ·ַ
				printf("HDD:Index=%d,LOCATE=%s\n",i,item_HDD->pData[C_LOCATE].c_str());
				i++;
				//if(item_HDD[i]!=NULL)  { delete item_HDD[i];}  
// 				item_HDD[i]=new ContentItem;
				break;
...
// 	setText(0, pData[C_STATUS].c_str());
	setTextAlignment(0, Qt::AlignRight);
	setText(0, pData[C_PROGRESS].c_str());
	setText(1, pData[C_NAME].c_str());
...
// 	setText(4, pData[C_STEREO].c_str());
	...
	QString s = pData[C_RECVSEGMENT].c_str();
	int n = s.size();
	for(int i = 1; i <= (n-1)/3; i++)
	{
		s.insert(n - i * 3, ",");
...
// int currentItem=(int)(ui.treeWidget_HDD_info->currentItem());
	...
	if(m_ConnectStatus==2)    //ȷѾӷ
	{
      ...
      //  char tmp_dst[256]="/root/copytest";
		...
		std::string stmp=((ContentItem*)ui.treeWidget_HDD_info->currentItem())->pData[C_LOCATE];
