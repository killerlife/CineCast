 ...
 *
 *	\version $Id: esp_jp2k.h,v 1.1.1.1 2007/04/03 02:08:42 baiqian Exp $
 *
 */
/*
...
namespace mxflib
{
	///baiqian add: for parse jpi file
	struct JPIIndex
	{
		UInt64 fileLen;
		UInt32 editUnitNum;
	...
	protected:
        ...
        Position m_Duration;                                //baiqian: wrap duration
		Position m_startUnit;                               //baiqian: wrap origin, skip start edit unit number
		bool m_isJpi;										//baiqian: is a jpi file?
		JPIIndex	m_jpiIndexInfo;
	public:
		...
		public:
			...
			{
				//baiqian:overwrite this, change to  encrypt data len;
			...
			virtual DataChunkPtr GetEssenceData(UInt64 Size = 0, UInt64 MaxSize = 0)
			{
				...
				if(!Started)
				{
					...
					EssenceBytePos = pCaller->DataStart;
			...
			//baiqian : overwrite this function, do encrpt here,and return new datachunk!
	...
	public:
		...
		//baiqian : sometimes , we only need part of essence;
	...
	protected:
        ...
        //baiqian: count the length of  Count frames fellowed; ,picNumber return actual frame number;
		Length CountPos(FileHandle InFile,Position Pos,Length Count,Position& picNumber);
		Length CountJpiPos(FileHandle InFile,Position Pos,Length Count,Position& picNumber);
		...
		bool ParseJpiFooter(FileHandle InFile);
